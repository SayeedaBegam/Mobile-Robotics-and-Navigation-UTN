...
def resample_particles(particles, weights):
    ...
    u = np.random.uniform(0, step)  # random start of first pointer
    c = weights[0]  # current weight value
    i = 0
...
# Replace half of the particles with the lowest weights with random particles
def add_random_particles(particles, weights, map_limits):
    weight_indices = np.argsort(np.array(weights))  # sort indices by weight
    n = len(particles)
    for i in range(n // 2):  # replace half of the particles with the lowest weights
